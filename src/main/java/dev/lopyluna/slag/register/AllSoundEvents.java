...
public class AllSoundEvents {
    ...
    public static final BlockSoundProvider CRUCIBLE = new BlockSoundProvider("crucible",
            "place", 5,
            "step", 5,
            "place", 5,
            "step", 5,
    ...
    private static SoundEntryBuilder create(String name) {
        return create(SlagEmbers.loc(name));
    }
    ...
    public static void provideLang(BiConsumer<String, String> consumer) {
        for (SoundEntry entry : ALL.values()) if (entry.hasSubtitle()) consumer.accept(entry.getSubtitleKey(), entry.getSubtitle());
    }
    ...
    public static SoundEntry createBlock(String name, String type, String id, int var) {
        ...
        for (var i = 0; (Math.max(var, 0))>i; i++) builder = builder.addVariant(name + "/" + id + (var-1==0 ? "" : i+1));
    ...
    public static class SoundEntryProvider implements DataProvider {
        ...
        public CompletableFuture<?> generate(Path path, CachedOutput cache) {
            path = path.resolve("assets/" + SlagEmbers.MOD_ID);
    ...
    public static class SoundEntryBuilder {
        ...
        public SoundEntryBuilder addVariant(String name) {
            return addVariant(SlagEmbers.loc(name));
        }
    ...
    private static class WrappedSoundEntry extends SoundEntry {
        ...
        public void play(Level level, Player player, double x, double y, double z, float volume, float pitch) {
            for (CompiledSoundEvent event : compiledEvents) level.playSound(player, x, y, z, event.event().get(), category, event.volume() * volume, event.pitch() * pitch);
        }

        @Override
        public void playAt(Level level, double x, double y, double z, float volume, float pitch, boolean fade) {
            for (CompiledSoundEvent event : compiledEvents) level.playLocalSound(x, y, z, event.event().get(), category, event.volume() * volume, event.pitch() * pitch, fade);
        }
